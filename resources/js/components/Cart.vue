<template>
    <div v-if="shoppingcart.length != 0" class="content">
        ...
        <div class="rounded"v-for="product in shoppingcart">
...
export default {
    ...
    computed:{
        total(){
            return this.shoppingcart.reduce(function(sum,product){
                return sum + (parseFloat(product.price)*parseFloat(product.qty))
            },0)
        }
    }
}
